...
SPANISH_WORDS='\b(el|la|los|las|del|una|unos|unas|para|con|que|por|como|este|esta|estos|desde|donde|entre|cada|todo|toda|hasta|pero|porque|solo|nombre|valor|ambiente|ambientes|nodos|subredes|cuenta|clave|estado|recurso|recursos|politica|politicas|modulo|servicios|cluster de|tipo de)\b'
